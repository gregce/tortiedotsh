...
| Agent Multiplexers | 14 | 69 | 69 |
| **Total** | **52** | **278** | **278** |

The generated ledger has exact key parity with all 278 current rendered Unknown cells. Every cell has a product-and-row-specific rationale and every product has an exact-SKU first-party `sourcesChecked` list. The generator rejects missing, extra, or duplicate keys.
