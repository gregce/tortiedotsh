    ...
    sections: [
      {
        ...
        blocks: [
          { type: "table", headers: ["Section", "What it controls"], rows: [
            ...
            ["Launch defaults", "Per-agent launch flags applied to each new session, and the shell variables your agents are given — named once for every agent, or per agent."],
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: "paragraph", html: "Launch defaults is also where you name the shell variables your agents need — an API key for a provider, say. Name them once under <strong>Every agent</strong> and every agent Tortie launches gets them, including agents you install later. Each agent's card says what it inherits, and you can still name variables on a single agent's card when you want that one to have more." },
          { type: "paragraph", html: "The list offers the names your own shell exports, so you pick rather than type. It scrolls, filters as you type, and takes several names in one go under one confirmation. A name your shell does not export yet is accepted too, so you can add it before you add the key." },
          { type: "note", title: "Names are kept, values never are", html: "Tortie stores the name and reads the value fresh from your login shell. A name applies to sessions you start after adding it, not to ones already running. A name only counts if you added it here: one written into the settings file by hand is ignored, because the agents you run can write that file too." },
          { type: "note", title: "When a rotated key does not arrive", html: "Tortie reads your shell once and watches your shell config files, so changing a key there takes effect on the next session you start with nothing to restart. A key that comes from somewhere those files do not show — a file your <code>.zshrc</code> sources, or a vault it calls — needs <strong>Re-read shell</strong>, next to the variables in Launch defaults." },
