...
Task and content: provide a direct getting-started path; explain what Tortie is and who it is for; explain Projects, Sessions, Splits, the private tmux server, Attention, Catch Me Up, recovery, session tools, native context menus, files, source control, agent context, skills, settings, remote work, supported agents, and the complete shortcut map; keep release notes at `/docs/changelog/`; retain an explicit link to GitHub from the uniform site header.

Constraints: claims must be supported by `/Users/gdc/gmux` or an explicit tested-release statement from the maintainer. Explain tmux as the private durability server without requiring tmux as a user workflow. Quitting and reboot recovery are described separately. The current release requires Apple silicon and has been tested on macOS 15.7.9 or later. There are 11 launchable agents; capture-only IDE watchers are not described as launchable agents. The comparison matrix remains unchanged apart from the shared header and Tortie's supported-platform note.
